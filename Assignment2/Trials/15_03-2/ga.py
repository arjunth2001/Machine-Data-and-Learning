...
def get_fitness(chromosomes):
    ...
    for chromosome in chromosomes:
        ...
        if minVal == None:
            ...
            if ((1e15 + ((minVal[0] + FACTOR * minVal[1])))/(((minVal[0] + FACTOR * minVal[1])) + abs(minVal[0]-minVal[1])*SUM_FACTOR)) <= ((1e15 + ((ta_answer[0] + FACTOR * ta_answer[1])))/(((ta_answer[0] + FACTOR * ta_answer[1])) + abs(ta_answer[0]-ta_answer[1])*SUM_FACTOR)):
                minVal = ta_answer
                minguy = chromosome
        fitness.append(
            (1e15 + ((ta_answer[0] + FACTOR * ta_answer[1])))/(((ta_answer[0] + FACTOR * ta_answer[1])) + abs(ta_answer[0]-ta_answer[1])*SUM_FACTOR))
...
def get_init(chromosome):
    ...
    for i in range(len(temp)):
        ...
        indices = np.random.choice(np.arange(
            temp[i].size), replace=False, size=np.random.randint(8, 10))
